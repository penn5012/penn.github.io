...
# 2027-04-15 · Day 227 · 幂等地创建工单

[全年索引](../../../README.md) · [分类索引](../../README.md) · [月度原计划](../../../../08-第8月-LangGraph工作流.md)

[上一天：有限重试与退避](../2027-04-14-Day226-有限重试与退避/README.md) · [下一天：补偿与部分失败处理](../2027-04-16-Day228-补偿与部分失败处理/README.md)

## 今日定位

- **日期**：2027-04-15（星期四）
- **阶段**：第 8 月 / 模块内第 17 天 / 全年第 227 天
- **分类**：LangGraph工作流
- **难度**：进阶
- **原计划时间**：2～2.5 小时。
...
幂等键、唯一约束、去重表、请求状态；“至少一次执行”环境中的副作用控制。

### 概念自检

- [ ] 能用自己的话解释：幂等键、唯一约束、去重表、请求状态
- [ ] 能用自己的话解释：“至少一次执行”环境中的副作用控制
...
使用 `requestId + actionType` 生成幂等键；在数据库建立唯一约束；创建工单前查询执行记录，成功后保存外部工单 ID。
...
同一请求并发提交或恢复两次只产生一个工单；返回相同工单 ID；有自动化测试证明而不是人工观察。
...
- 分支：`study/day-227`
- 建议提交：`study(day-227): 幂等地创建工单`
- 提交内容应包括：代码或配置、必要测试、当天笔记、验收证据索引。

## 下一步

[下一天：补偿与部分失败处理](../2027-04-16-Day228-补偿与部分失败处理/README.md)

...
# 2027-04-13 · Day 225 · 统一超时与错误分类

[全年索引](../../../README.md) · [分类索引](../../README.md) · [月度原计划](../../../../08-第8月-LangGraph工作流.md)

[上一天：第二周故障恢复演练](../2027-04-12-Day224-第二周故障恢复演练/README.md) · [下一天：有限重试与退避](../2027-04-14-Day226-有限重试与退避/README.md)

## 今日定位

- **日期**：2027-04-13（星期二）
- **阶段**：第 8 月 / 模块内第 15 天 / 全年第 225 天
- **分类**：LangGraph工作流
- **难度**：进阶
- **原计划时间**：2 小时。
...
调用超时、可重试错误、不可重试错误、业务错误、取消信号；错误码与用户提示分离。

### 概念自检

- [ ] 能用自己的话解释：调用超时、可重试错误、不可重试错误、业务错误、取消信号
- [ ] 能用自己的话解释：错误码与用户提示分离
...
为 LLM、RAG、工单和通知调用封装统一超时；定义错误类型与状态字段；在图中加入错误路由节点。
...
模拟超时、401、429、5xx 和非法数据；每类错误走到正确路径；前端不会看到堆栈或内部密钥信息。
...
- 分支：`study/day-225`
- 建议提交：`study(day-225): 统一超时与错误分类`
- 提交内容应包括：代码或配置、必要测试、当天笔记、验收证据索引。

## 下一步

[下一天：有限重试与退避](../2027-04-14-Day226-有限重试与退避/README.md)

...
# 2026-11-24 · Day 085 · 重复请求与幂等性

[全年索引](../../../README.md) · [分类索引](../../README.md) · [月度原计划](../../../../03-第3月-NodeAI后端.md)

[上一天：限流、输入边界与成本保护](../2026-11-23-Day084-限流-输入边界与成本保护/README.md) · [下一天：健康检查与优雅停机](../2026-11-25-Day086-健康检查与优雅停机/README.md)

## 今日定位

- **日期**：2026-11-24（星期二）
- **阶段**：第 3 月 / 模块内第 25 天 / 全年第 85 天
- **分类**：Node.js AI后端
- **难度**：基础工程
- **原计划时间**：2～2.5 小时。
...
客户端请求 ID；唯一约束；幂等键；“至少一次”请求带来的重复副作用。
...
客户端每次发送生成 `clientMessageId`；消息表添加对应唯一约束；服务端收到相同用户、会话和 clientMessageId 时返回已有结果或冲突状态；模拟双击和网络重发。
...
同一请求并发发送两次，数据库只出现一条用户消息且只启动一次模型任务；不同请求仍可正常发送。
...
- 分支：`study/day-085`
- 建议提交：`study(day-085): 重复请求与幂等性`
- 提交内容应包括：代码或配置、必要测试、当天笔记、验收证据索引。

## 下一步

[下一天：健康检查与优雅停机](../2026-11-25-Day086-健康检查与优雅停机/README.md)

...
# 2027-05-19 · Day 261 · 验证幂等、重放与审批安全

[全年索引](../../../README.md) · [分类索引](../../README.md) · [月度原计划](../../../../09-第9月-Agent与MCP.md)

[上一天：实现高风险操作人工审批](../2027-05-18-Day260-实现高风险操作人工审批/README.md) · [下一天：理解 MCP 架构与适用场景](../2027-05-20-Day262-理解-MCP-架构与适用场景/README.md)

## 今日定位

- **日期**：2027-05-19（星期三）
- **阶段**：第 9 月 / 模块内第 21 天 / 全年第 261 天
- **分类**：Agent与MCP
- **难度**：进阶
- **原计划时间**：2.5 小时。
...
写工具需使用幂等键；审批应绑定调用参数摘要、用户、会话和有效期；参数被修改后应重新校验风险与权限。

### 概念自检

- [ ] 能用自己的话解释：写工具需使用幂等键
- [ ] 能用自己的话解释：审批应绑定调用参数摘要、用户、会话和有效期
- [ ] 能用自己的话解释：参数被修改后应重新校验风险与权限
...
模拟双击批准、网络重试、旧审批链接、修改订单号后批准四类场景。
...
每个幂等键最多创建一个工单；过期审批拒绝执行；参数变化可被审计并重新校验。
...
- 分支：`study/day-261`
- 建议提交：`study(day-261): 验证幂等-重放与审批安全`
- 提交内容应包括：代码或配置、必要测试、当天笔记、验收证据索引。

## 下一步

[下一天：理解 MCP 架构与适用场景](../2027-05-20-Day262-理解-MCP-架构与适用场景/README.md)

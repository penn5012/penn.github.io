...
# 2027-05-14 · Day 256 · 实现显式长期记忆

[全年索引](../../../README.md) · [分类索引](../../README.md) · [月度原计划](../../../../09-第9月-Agent与MCP.md)

[上一天：长期记忆的边界与数据模型](../2027-05-13-Day255-长期记忆的边界与数据模型/README.md) · [下一天：长期记忆检索与隔离](../2027-05-15-Day257-长期记忆检索与隔离/README.md)

## 今日定位

- **日期**：2027-05-14（星期五）
- **阶段**：第 9 月 / 模块内第 16 天 / 全年第 256 天
- **分类**：Agent与MCP
- **难度**：进阶
- **原计划时间**：2 小时。
...
长期记忆写入应有明确触发、校验和用户可见反馈；默认不从每轮对话自动提取一切信息。

### 概念自检

- [ ] 能用自己的话解释：长期记忆写入应有明确触发、校验和用户可见反馈
- [ ] 能用自己的话解释：默认不从每轮对话自动提取一切信息
...
实现 `rememberPreference` 和 `deleteMemory` 两个受控能力；支持用户说“记住我偏好短信联系”和“删除这个偏好”。
...
跨新会话可读取偏好；用户可查看和删除；写入记录含来源；敏感信息默认拒绝写入。
...
- 分支：`study/day-256`
- 建议提交：`study(day-256): 实现显式长期记忆`
- 提交内容应包括：代码或配置、必要测试、当天笔记、验收证据索引。

## 下一步

[下一天：长期记忆检索与隔离](../2027-05-15-Day257-长期记忆检索与隔离/README.md)

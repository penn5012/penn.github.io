...
# 2026-09-14 · Day 035 · 优化流式渲染性能

[全年索引](../../../README.md) · [分类索引](../../README.md) · [月度原计划](../../../../02-第2月-AI交互体验.md)

[上一天：封装流式数据解析层](../2026-09-13-Day034-封装流式数据解析层/README.md) · [下一天：实现停止生成与资源清理（周末整合）](../2026-09-15-Day036-实现停止生成与资源清理（周末整合）/README.md)

## 今日定位

- **日期**：2026-09-14（星期一）
- **阶段**：第 2 月 / 模块内第 5 天 / 全年第 35 天
- **分类**：AI交互与多模态
- **难度**：入门
- **原计划时间**：2 小时。
...
批量刷新、`requestAnimationFrame`、局部状态、列表 memo、自动滚动的用户意图判断。

### 概念自检

- [ ] 能用自己的话解释：批量刷新、`requestAnimationFrame`、局部状态、列表 memo、自动滚动的用户意图判断
...
把 token 缓冲后按帧或固定间隔更新；仅在用户位于底部附近时自动滚动；用户向上查看历史时显示“回到最新”。
...
用长文本流验证页面不明显卡顿；向上滚动时不会被强制拉回底部；记录一次优化前后对比。
...
- 分支：`study/day-035`
- 建议提交：`study(day-035): 优化流式渲染性能`
- 提交内容应包括：代码或配置、必要测试、当天笔记、验收证据索引。

## 下一步

[下一天：实现停止生成与资源清理（周末整合）](../2026-09-15-Day036-实现停止生成与资源清理（周末整合）/README.md)

...
将 `backend/.env.example` 复制为 `backend/.env`，只在服务端填写
`OPENAI_API_KEY`；密钥不会被前端接收，也不能提交到 Git。默认模型为
`gpt-5.4-nano`，请求超时默认为 `30000` 毫秒。未配置密钥时服务仍可启动，
...
curl -X POST "http://localhost:3000/api/conversations/${conversation_id}/messages" \
  -H 'content-type: application/json' \
  -d '{"content":"请用通俗的语言解释 Node.js 事件循环。"}'
```
